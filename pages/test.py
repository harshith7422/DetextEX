...
def test_page():
    ...
    if not st.session_state.test_started:
        ...
        if st.button("Submit Test"):
            score = sum(1 for q_id, _, _, correct_option in questions if str(st.session_state.answers.get(q_id, "")).strip().lower() == str(correct_option).strip().lower())
            save_test_attempt(st.session_state.name, st.session_state.reg_number, st.session_state.college, score)
